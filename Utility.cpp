...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    {
        dw->boostArmor( static_cast<int>(item.getBoost() * 1.1 ));
    }
    else if( auto* pl = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        pl->boostArmor( static_cast<int>(item.getBoost() * 1.3));
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
        ds->boostArmor( static_cast<int>(item.getBoost() * 1.5 ));
    }
    else if( auto* d = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    ...
    {
        pl->boostHitPoints(static_cast<int>(item->getBoost() * 1.5));
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
        ds->boostHitPoints(static_cast<int>(item->getBoost() * 1.25));
    }
    else if( auto* d = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    if( auto* dw = dynamic_cast<Dwarf*>(character) )
    {
        dw->boostAttackDamage(static_cast<int>(item->getBoost() * 1.5));
    }
    else if( auto* pl = dynamic_cast<Paladin*>(character) )
    {
        pl->boostAttackDamage(static_cast<int>(item->getBoost() * 1.33));
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
